	...
	if ("undefined" == typeof window.powerbiframehandlerscript) {
		...
		var e = function() {
			for (var e = document.querySelectorAll('.pbi-resize-container'), i = 0; i < e.length; i++) {
				var doResizeFromImg = e[i].getAttribute("pbi-resize-ratio-from-img");
				if ( doResizeFromImg ) {
					var img = getChildByTag(e[i], 'img');
					var that = e;
					var iThat = i;
					img.onload = (eve) => {
						var curImg = eve.path[0];
						var mobileCut = that[iThat].getAttribute("pbi-resize-mobile-min-width");
						if ( getPageWidth() > mobileCut ) {
							curImg.parentNode.setAttribute('pbi-resize-width', curImg.naturalWidth);
							curImg.parentNode.setAttribute('pbi-resize-height', curImg.naturalHeight);
						} else {
							curImg.parentNode.setAttribute('pbi-resize-m-width', curImg.naturalWidth);
							curImg.parentNode.setAttribute('pbi-resize-m-height', curImg.naturalHeight);
						}
						prepareForResize(that, iThat);
					}
				}
				prepareForResize(e, i);
			}
		};

		function prepareForResize(e, i) {
